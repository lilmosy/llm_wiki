...
- **cover**(예측이 정답을 포함하는가): LLM-Wiki 8/8 vs baseline 7/8. **LLM-Wiki가 사실상 전 문항 정답**, baseline은 **q1(4-hop)에서 오답** — 논문 Appendix H Case 1의 실패를 그대로 재현.
- **F1/EM**에서 LLM-Wiki가 낮게 보이는 건 **지식 부족이 아니라 에이전트가 문장형으로 답해 토큰 overlap이 깎이는 harness 아티팩트** (논문 AuthTrace의 'Single-doc에서 위키가 지는' 현상과 동형: 위키가 사실을 재조직해 국소 표현이 달라짐).
- **쟁점(고려 필요)**: 이 verbose 아티팩트를 없애려면 양쪽 프롬프트를 terse-span(짧은 정답 스팬)으로 강제하는 교정이 필요합니다(코드엔 반영됨). 그 교정본 **재실행 전**이라 아래 F1/EM은 교정 전(verbose) 수치이며, 이 세팅에서 **F1/EM과 cover 중 무엇을 정확도 기준으로 삼을지**가 열린 논점입니다 — 본 리포트는 장황함에 강건한 **cover를 1차 지표**로 봅니다.

## Phase 1 — 인덱스 타임 컴파일 (Algorithm 1)

- 입력 passage **16개** → 컴파일된 위키 페이지 **19개** (passage:page ≠ 1:1 — 엔티티 중심 재조직으로 여러 passage가 한 엔티티에 모이고 한 passage가 여러 엔티티로 분배됨)
...
| p04 | 0 | Herschell-Gordon-Lewis, Monster-a-Go-Go, Bill-Rebane | Herschell-Gordon-Lewis, Monster-a-Go-Go, Chicago | – |
| p05 | 1 | Bill-Rebane, Monster-a-Go-Go, Herschell-Gordon-Lewis | Bill-Rebane | – |
| p06 | 1 | Monster-a-Go-Go, The-Gamecock-film | The Mask of the Gorilla | – |
| p07 | 1 | Bill-Rebane | The Capture of Bigfoot, Bill Rebane, The Gamecock | – |
...
| p10 | 2 | Ernest-I-Prince-of-Anhalt-Dessau, House-of-Ascania, John-V-Prince-of-Anhalt-Zerbst | Ernest I, Prince of Anhalt-Dessau, House of Ascania | – |
| p11 | 2 | Karl-I-Prince-of-Anhalt-Zerbst, John-V-Prince-of-Anhalt-Zerbst, House-of-Ascania | Karl I, Prince of Anhalt-Zerbst, John V, Prince of Anhalt-Zerbst, Anhalt-Zerbst | – |
| p12 | 2 | John-V-Prince-of-Anhalt-Zerbst | Margarete of Munsterberg, Henry I, Duke of Munsterberg-Oels, John V, Prince of Anhalt-Zerbst | – |
| p13 | 3 | House-of-Ascania, Anhalt-Zerbst, Ernest-I-Prince-of-Anhalt-Dessau, John-V-Prince-of-Anhalt-Zerbst, Karl-I-Prince-of-Anhalt-Zerbst | House of Ascania | – |
| p14 | 3 | John-V-Prince-of-Anhalt-Zerbst | Joachim I Nestor, Elector of Brandenburg, Margaret of Brandenburg, John V, Prince of Anhalt-Zerbst | – |
...
|---|---|---|---|---|---|---|---|---|
| q1 | 4 | bridge_comparison | Monster a Go-Go | Monster a Go-Go (co-director Hersc | ✅ | 4 | The Gamecock | ❌ |
| q2 | 2 | bridge | 12 June 1516 | John V's father, Ernest I, Prince  | ✅ | 2 | 12 June 1516 | ✅ |
| q3 | 2 | comparison | Herschell Gordon Lewis | Herschell Gordon Lewis (born June  | ✅ | 3 | Herschell Gordon Lewis (born | ✅ |
| q4 | 2 | compositional | Dessau | Dessau | ✅ | 2 | Dessau | ✅ |
| q5 | 2 | bridge | Herschell Gordon Lewis | Herschell Gordon Lewis (known as t | ✅ | 2 | Herschell Gordon Lewis | ✅ |
| q6 | 1 | direct | Pasquale Festa Campanile | Pasquale Festa Campanile | ✅ | 2 | Pasquale Festa Campanile | ✅ |
| q7 | 2 | comparison | The Gamecock | The Gamecock (1964) was released e | ✅ | 3 | The Gamecock (1964) | ✅ |
...
- 경로: wiki_search(The Gamecock film) → wiki_search(Monster a Go-Go film) → wiki_read(['The-Gamecock-film', 'Monster-a-Go-Go']) → wiki_read(['Pasquale-Festa-Campanile', 'Bill-Rebane', 'Herschell-Gordon-Lewis'])
- 브리지 엔티티('campanile') 도달: ✅ | 예측='Monster a Go-Go (co-director Herschell Gordon Lewis, born 19' → cover ✅
...
- 브리지 엔티티('ernest') 도달: ✅ | 예측='John V's father, Ernest I, Prince of Anhalt-Dessau, died on ' → cover ✅
...
| Single-doc 국소 detail | 위키가 재조직해 국소 표현 손실 | 위키 verbose→F1↓(동형) |
| 효율 | 위키 평균 2.5~3.9 페이지 read | 평균 2.5 tool calls |

> ⚠️ **한계/재현 주의**: (1) 코퍼스 8문항 소규모라 절대 수치의 통계적 의미는 제한적 — 목표는 절대 F1 복제가 아니라 **파이프라인 동작 + Appendix H 트레이스 재현 + 위키>flat·hop깊을수록 우위 패턴**의 확인. (2) 컴파일은 LLM 비결정성으로 실행마다 페이지 수가 약간 변동(run1 21 / run2 19). (3) F1 verbosity 교정(terse-span 강제)은 코드에 반영됐으나 교정본 재실행 전이라 F1/EM은 교정 전 수치 — 정확도는 cover로 판정(0절 쟁점 참조). 실제 2Wiki dev 앞 50개로 스케일업은 `data/corpus.jsonl`·`questions.json` 교체만으로 가능(로더 동일).

## 비용 (compile-time vs query-time)

- 총 LLM 호출 **80회** | 입력 토큰 69,895 | 출력 토큰 12,075
...
- 코드: `indexing/`(compile·validators·error_book) `retrieval/`(agent·tools=wiki.py) `baseline/` `harness/`